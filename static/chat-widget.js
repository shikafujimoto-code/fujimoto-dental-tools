(function () {
  ...
  style.textContent = `
    ...
    #fdc-widget { font-family: 'Hiragino Sans', 'Meiryo', 'Yu Gothic', sans-serif; isolation: isolate; contain: layout style; }
  ...
  wrap.setAttribute('aria-label', 'AIチャットアシスタント');
  wrap.innerHTML = `
    <button id="fdc-btn" aria-label="チャットを開く" title="ふじもと歯科 AIチャット"><img src="${BASE_URL}/AI.png" alt="AIに質問"></button>
  ...
  async function send() {
    ...
    try {
      ...
      addBot(data.reply);
      if (!reservationShown && (showReservation || hasReservationKeyword(data.reply))) {
      ...
      if (!reservationShown && showReservation) { addReservationButtons(); reservationShown = true; }
    } finally {
      loading = false;
